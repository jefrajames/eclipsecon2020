...
public class MerchantHateoasDecorator {
    ...
    public JsonObject addLinksToMerchant(UriInfo uriInfo, Merchant merchant) {
        ...
        JsonObject self = Json.createObjectBuilder().add("self", getMerchantUri(uriInfo, merchant.getId()).toString()).build();
        JsonArrayBuilder linksBuilder = Json.createArrayBuilder().add(self);

        if (merchant.getScoreEnabled()) {
            JsonObject scores = Json.createObjectBuilder().add("scores", scoreHateoasDecorator.getScoresUri(uriInfo, merchant.getScoreId()).toString()).build();
